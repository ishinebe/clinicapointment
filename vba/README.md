...
11. アポ帳生成時はフォームの「この内容でアポ帳を作成」ボタン、または従来の `Settings` の「アポ帳を作成」ボタンを押す。
...
5. フォームで年・月、担当者、毎週の休み・早上がり、医院全体の当月原則終了時刻を順に確認する。
6. 必要なら「臨時予定を確認・編集」から特定日だけの休診・早上がり・スタッフ休みを設定する。
7. フォームの「この内容でアポ帳を作成」ボタンを押す。
8. Output の見た目、網掛け、改ページ、印刷範囲を確認する。
```

今後、入力チェックマクロを追加した場合は、`GenerateBook_Phase5_Print` の前に入力チェックを実行する。
...
`CreateAppointmentBookButton` は、`Settings` シートに `GenerateBook_Phase5_Print` を実行する「アポ帳を作成」ボタンを配置する。
...
`ThisWorkbook.Workbook_Open` でも `ShowAppointmentSettingsForm` の表示を予約するため、マクロ有効状態でExcelファイルを開くと月次アポ帳作成フォームが自動で開く。フォームを閉じても、`Settings` シートの「設定フォームを開く」ボタンまたは `Alt + F8` の `ShowAppointmentSettingsForm` から再度開ける。
...
フォームの「保存して閉じる」は、入力値を `Settings` へ書き戻してフォームを閉じる。

フォームの「この内容でアポ帳を作成」は、入力値を `Settings` へ保存してから `GenerateBook_Phase5_Print` を実行する。

フォームの「臨時予定を確認・編集」は、フォーム上の年月を `Settings` へ保存してから `SetupExceptionsDateDropdowns` を実行し、`frmTemporarySchedule` を開く。
