...
4. `ファイル` → `ファイルのインポート` から `.bas` / `.frm` ファイルを選択する。
5. 初回設定として `Alt + F8` で `SetupUserFriendlySettings` または `SetupSettingsDropdowns` を実行する。
6. `Alt + F8` で `ShowAppointmentSettingsForm` を実行するか、`Settings` の「設定フォームを開く」ボタンを押す。
7. アポ帳生成時はフォームの「アポ帳を作成」ボタン、または従来の `Settings` の「アポ帳を作成」ボタンを押す。
...
4. 必要なら Exceptions で特定日だけの休診・時短診療・スタッフ休み・スタッフ時短勤務を設定する。
...
- `Settings` シートに「アポ帳を作成」ボタンと「例外日付を更新」ボタンを配置する。
...
`CreateRefreshExceptionDatesButton` は、`Settings` シートに `SetupExceptionsDateDropdowns` を実行する「例外日付を更新」ボタンを配置する。
...
VBEへのインポート互換性を優先するため、`frmAppointmentSettings.frm` はASCII英数字中心で管理する。日本語Captionや日本語MsgBoxは後続フェーズで、文字コード方針を決めてから追加する。
...
フォームの「例外日付を更新」は、フォーム上の年月を `Settings` へ保存してから `SetupExceptionsDateDropdowns` を実行する。

既存の `Settings` シート直接入力と既存ボタンも引き続き使用できる。

### インポート時の注意

Excel VBEは `.frm` 内の日本語文字列をUTF-8として正しく解釈できないことがある。そのため、GitHub上の `frmAppointmentSettings.frm` には日本語UI文字列を直接含めない。

`.frm` をインポートできない場合は、いったん該当UserFormをVBE上から削除し、`vba/frmAppointmentSettings.frm` を再インポートする。
