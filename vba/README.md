...
6. 年月を変更した後、必要に応じて `Settings` の「例外日付を更新」ボタンを押す。
7. アポ帳生成時は `Settings` の「アポ帳を作成」ボタンを押す。
...
3. Settings で担当者をプルダウンから選択する。
4. 必要なら曜日別勤務パターンを設定する。
5. 必要なら Exceptions で特定日だけの休診・時短診療・スタッフ休み・スタッフ時短勤務を設定する。
6. Settings の「アポ帳を作成」ボタンを押す。
7. Output の見た目、網掛け、改ページ、印刷範囲を確認する。
```
...
現在はUI改善フェーズとして、`Settings` シートを受付・歯科助手向けの操作画面として整備する。大きな見出し、説明文、入力セルの色分け、プルダウン、作成ボタンを配置し、現場で迷いにくい見た目にする。
