...
現在は、受付・歯科助手でも迷わず操作できるように、`frmAppointmentSettings` を月次アポ帳作成ウィザードとして、`frmTemporarySchedule` を臨時予定フォームとして追加しています。UserFormは `Settings` シートを置き換えるものではなく、既存セルへ入力する補助画面です。既存ロジックとの互換性維持のため、内部参照セルである `B2`、`B3`、`B5:F5`、`B7:F13`、`B16:F16`、`H5:H24` は変更しません。

印刷設定については、`vba/AppointmentBook_Phase1.bas` の `GenerateBook_Phase5_Print` に統合しています。これにより、行高コピー、印刷範囲設定、横1ページ固定、日別ブロックごとの手動改ページ、網掛け開始セルの上罫線追加をまとめて実行できます。

フォーム側と `Settings` シート上の作成ボタンは、短い正式入口 `GenerateBook_Phase5_Print` を呼び出します。従来の `GenerateAppointmentBook_Phase5` は互換用ラッパーとして残し、同じ入口へ委譲します。
...
5. フォームで年月、担当者、毎週の休み・早上がり、医院全体の終了時刻を順に確認する。
6. 必要ならフォームの「臨時予定を確認・編集」から休診・早上がりなどを入力する。
7. フォームの「この内容でアポ帳を作成」ボタンを押す。
8. `Output` シートを改ページプレビューで確認し、1日分が1ページに収まっているか確認する。

`Alt + F8` から直接実行する場合は、`GenerateBook_Phase5_Print` を実行してください。

従来通り、`Settings` シートを直接編集して作成ボタンから作成する運用も残しています。作成ボタンの呼び出し先は `GenerateBook_Phase5_Print` です。
...
`GenerateBook_Phase5_Print` は、`Output` 生成時に以下を行います。
...
`GenerateBook_Phase5_Print` は、アポ帳生成と印刷設定をまとめて実行する入口です。フォーム・Settingsボタンとも、このマクロを呼びます。
