...
- `frmAppointmentSettings` による月次アポ帳作成フォームを追加済み。
- `frmTemporarySchedule` による臨時予定フォームを追加済み。
- `PrintPageSetup.bas` を追加し、行高再正規化、印刷範囲設定、1日1ページの改ページ設定、網掛け開始セルの上罫線追加を実装済み。

### 注意点

- `PrintPageSetup.bas` の統合マクロは追加済みだが、フォームや `Settings` シート上の既存ボタンがまだ従来の `GenerateAppointmentBook_Phase5` を呼んでいる場合、印刷設定・罫線処理が適用されない。
- 起動時フォーム表示は `ThisWorkbook.cls` の `Workbook_Open` 貼り付けが前提であり、実ファイル側での反映確認が必要。
- UserFormの見た目はまだVBA標準フォーム寄りで、現場向けUIとして改善余地がある。
...
受付・歯科助手が初見でも迷わないように、フォームを「月次作成ウィザード」に近い構成へ整理する。
...
すでに `Output` が生成済みで、網掛け開始セルの罫線だけ追加したい場合は以下を実行する。

```vba
ApplyShadeStartTopBordersToOutput
```

すでに `Output` が生成済みで、行高・改ページ・印刷範囲まで整えたい場合は以下を実行する。
